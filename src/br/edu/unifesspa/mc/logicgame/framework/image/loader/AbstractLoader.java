...
import br.edu.unifesspa.mc.logicgame.framework.image.ImageChanger;
...
import br.edu.unifesspa.mc.logicgame.framework.image.ImageWorker;
...
public abstract class AbstractLoader implements ImageLoader {
    ...
    protected ImageItem loadImageElement(Element imageElem) {
        ...
        Element changeElem = (Element) imageElem.getElementsByTagName(
                CHANGE_TAG).item(0);
    ...
    /**
     * Reads an integer attribute from the given element. If the attribute is
     * empty, returns <code>blankValue</code>. Numbers are automatically parsed.
     * All other values will result in a <code>NumberFormatException</code>.
     *
     * @param source The element to read from.
     * @param name The attribute name.
    ...
    protected ImageChanger loadChanger(Element changerElem) {
        ...
        return new ImageChanger(readIntAttribute(changerElem, "width"),
                readIntAttribute(changerElem, "height"), readFloatAttribute(
                        changerElem, "rotation"),
