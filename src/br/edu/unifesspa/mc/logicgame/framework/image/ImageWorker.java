package br.edu.unifesspa.mc.logicgame.framework.image;
 ...
 *
 * @author Mendon�a, Vin�cius Godoy de
 */
public class ImageWorker {
    ...
    public BufferedImage makeCompatible(BufferedImage sourceImage) {
        BufferedImage image = ScreenManager.getInstance()
                .createCompatibleImage(sourceImage.getWidth(),
                        sourceImage.getHeight(), sourceImage.getTransparency());
    ...
    public BufferedImage[] splitImage(BufferedImage img, int columns, int lines) {
        ...
        for (int col = 0; col < columns; col++) {
            for (int lin = 0; lin < lines; lin++) {
                BufferedImage copy = ScreenManager.getInstance()
    ...
    public BufferedImage rotateImage(BufferedImage source, double angle) {
        ...
        BufferedImage dest = ScreenManager.getInstance()
        ...
        rotation.rotate(angle, source.getWidth() / 2, source.getHeight() / 2);
    ...
    public ImageItem rotateImage(ImageItem source, double angle) {
        if (source == null) {
            throw new IllegalArgumentException("Source cannot be null!");
        }

        BufferedImage dest = ScreenManager.getInstance().createCompatibleImage(source.getWidth(), source.getHeight(),
                source.getImage().getTransparency());
        Graphics2D surface = dest.createGraphics();
        ...
        surface.drawImage(source.getImage(), 0, 0, null);

        surface.setTransform(originalTransform);
        surface.dispose();

        return new ImageItem(dest.toString(), dest);
    }

    public BufferedImage changeAlpha(BufferedImage image, float alpha) {
        BufferedImage other = ScreenManager.getInstance()
